app.controller('messageCtrl', function ($rootScope, $scope, $http, $state, $stateParams, $location, $modal) {
	...
	$scope.delete = function (id) {
		$http.post('/server/bannerService!delete', { _id: id }).success(function (data) {
			if (data.success) {
				$scope.search.flush = new UUID();
				allen.alert("删除1成功");
			}
...
app.controller('createBannerCtrl', function ($scope, $modalInstance, $http, item) {
	$scope.obj = {};
	$scope.ok = function () {
		$http.post('/server/bannerService!saveEntity', $scope.obj).success(function (data) {
			if (data.success) {
				allen.alert("发送成功");
...
app.controller('sendPicTextCtrl', function ($scope, $modalInstance, $http, item) {
	...
	$scope.send = function () {
		$http.post('/service/weixin-message!saveEntity', $scope.msg).success(function (data) {
			if (data.success) {
				allen.alert("发送成功");
...
//# sourceMappingURL=messageCtrl-compiled.js.map