			...
			if (options.enableButtons) {
				for (n in buttons) {
					if (buttons.hasOwnProperty(n)) {
						...
						button = chart.renderer.button(buttonOptions.text, 0, 0, outerHandler).attr({
							width: buttonOptions.width,
							height: buttonOptions.height
						}).css(buttonOptions.style).add();
  ...
  */
   ...
   */
		drawLegendSymbolGradient: function (legend, item) {
			...
			item.fromText = this.chart.renderer.text(item.fromLabel, // Text.
			...
			item.legendSymbol = this.chart.renderer.rect(horizontal ? box1.x + box1.width + spacing : box1.x - gradientSize - spacing, // Upper left x.
			box1.y, // Upper left y.
			...
			item.toText = this.chart.renderer.text(item.toLabel, box2.x + box2.width + spacing, horizontal ? positionY : box2.y + box2.height - spacing).attr({
				zIndex: 2
			}).add(item.legendGroup);
...
//# sourceMappingURL=map.src-compiled.js.map